...
begin
	...
	if (RandNumber2 = 1) then 
		if (RaceIsDunmer = True) then 
			if (RandSubNumberMale = 1) then
				SetEditValue(VoiceType, 'DLC2MaleDarkElfCommoner [VTYP:040247E4]');
		
	if (RandNumber2 = 1) then 
		if (RaceIsDunmer = True) then 
			if (RandSubNumberMale = 2) then
				SetEditValue(VoiceType, 'DLC2MaleDarkElfCynical [VTYP:04018469]');
		
	if (RandNumber2 = 1) then 
		if (RaceIsDunmer = True) then 
			if (RandSubNumberMale = 3) then
				SetEditValue(VoiceType, 'DLC2MaleDarkElfCommoner [VTYP:040247E4]');
		
	if (RandNumber2 = 1) then 
		if (RaceIsDunmer = True) then 
			if (RandSubNumberMale = 4) then
...
begin
  if Signature(e) <> 'NPC_' then
	...
	if (IsTemplateTraitsFlag = 0) then
